...
async function fetchTeamBanner() {
    try {
        ...
        if (contentType && contentType.includes('image/')) {
            return { 
                url: `${api}/Team/${teamId}/banner?token=${encodeURIComponent(getCookie("token"))}`,
...
function setupBannerUpload() {
    ...
    saveBtn.onclick = async function(event) {
        if (selectedBannerFile) {
            const bannerContainer = document.getElementById('hosted-team-banner');
            bannerContainer.innerHTML = '<div class="uploading-banner">Uploading banner...</div>';
            ...
            try {
                ...
                console.error('Error uploading banner:', error);
                alert('Failed to upload banner. The team information will still be saved.');
            }
        }
...
async function displayRoleAssignments(requirements, submissions) {
    ...
    requirements.forEach(requirement => {
        ...
        if (roleSubmissions.length > 0) {
            roleSubmissions.forEach(submission => {
                const isApproveDisabled = submission.status === "Pending" ? 'disabled' : '';
                const isRejectDisabled = submission.status === "Rejected" ? 'disabled' : '';
    ...
    document.querySelectorAll('.approve-btn').forEach(button => {
        button.addEventListener('click', function() {
            ...
            selectSubmissionStatus(submissionId, STATUS_PENDING);
        });
    });
    ...
    document.querySelectorAll('.reject-btn').forEach(button => {
        button.addEventListener('click', function() {
            ...
            selectSubmissionStatus(submissionId, STATUS_REJECTED);
        });
    });
}

async function selectSubmissionStatus(submissionId, selectedStatus) {
    const approveBtn = document.querySelector(`.approve-btn.submission-${submissionId}`);
    const rejectBtn = document.querySelector(`.reject-btn.submission-${submissionId}`);
    ...
    try {
        ...
        alert(selectedStatus === "2" ? "Application approved successfully!" : "Application rejected successfully!");
...
async function sendNotification(submission, teamId) {
  try {
    ...
    console.log("User: ", submission.user.id);
...
function validateRequirements(requirements, submissions) {
  for (const req of requirements) {
    const pendingCount = submissions.filter(
      submission => submission.requirement === req.id && submission.status === STATUS_PENDING
        ).length;
