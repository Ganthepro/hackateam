...
async function SearchSkill(inputElement, datalistId) {
    ...
    const body = { title: query, limit: 2 };
    console.log(body);
    try {
        ...
        inputElement.addEventListener("change", function () {
            const selectedOption = [...datalist.options].find(option => option.value === inputElement.value);
            const hiddenSkillInput = inputElement.closest(".member-requirement-container").querySelector('input[name="selected-skill-id"]');
            hiddenSkillInput.value = selectedOption ? selectedOption.dataset.skillId : "";
        });
...
function createRequirementGroup() {
    ...
    const skillInput = requirementContainer.querySelector('input[name="member-skill"]');
...
async function createRequirements(teamId, token) {
    ...
    for (const container of requirementContainers) {
        ...
        const maxSeat = parseInt(container.querySelector('input[name="member-quantity"]').value, 10);
        const skillTitle = container.querySelector('input[name="member-skill"]').value;
        let skillId = container.querySelector('input[name="selected-skill-id"]').value;
        ...
        try {
            ...
            if (!requirementResponse.ok) {
                throw new Error(`Failed to create requirement: ${requirementResponse.status}`);
            }
        } catch (error) {
